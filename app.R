...
# NEW: Create maps for weather colors and icons
WEATHER_COLORS <- list(
  "Clear" = "teal",
  "Rain" = "aqua",
  "Thunderstorm" = "blue",
...
fetch_all_mutation_data <- function() {
  message("--- Fetching ALL Mutation Data from growagardencalculator.net ---")
  ...
  tryCatch({
    ...
    message("   ERROR fetching ALL mutation data: ", e$message)
    return(tibble())
  })
}

# --- Data Preparation at Startup ---
all_mutation_data <- fetch_all_mutation_data()
mutation_data_for_calculator <- all_mutation_data %>%
  rename(bonus = stack_bonus) %>%
  select(name, bonus) %>%
  mutate(bonus = str_replace(bonus, "\\+", ""), 
...
server <- function(input, output, session) {
  ...
  # NEW: Add a flag for the 3:00 minute mark refresh.
  three_minute_mark_refresh_done <- reactiveVal(FALSE)
  ...
  observe({
    ...
    if (any(was_running & is_finished, na.rm = TRUE)) {
      ...
      # NEW: Reset the 3:00 minute flag when the cycle completes.
      three_minute_mark_refresh_done(FALSE)
      fetch_data()
    ...
    # NEW: Add a refresh at 3:00 (180s)
    if (!is.null(seed_timer_val) && seed_timer_val == 180 && !isolate(three_minute_mark_refresh_done())) {
      message("Seed/Gear timer hit 3:00. Triggering scheduled refresh.")
      showNotification("Performing 3-minute mark refresh...", type = "message", duration = 4)
      fetch_data()
      three_minute_mark_refresh_done(TRUE)
    }
  })
  ...
  output$weather_box <- renderValueBox({
    data <- scraped_data(); req(data)
    current_weather_name <- data$weather
    
    if (identical(current_weather_name, "API Error")) {
      return(valueBox("API OFFLINE", "Current Weather", icon=icon("exclamation-triangle"), color="red"))
    }
    
    # NEW: Dynamically select color and icon based on weather name
    box_color <- WEATHER_COLORS[[current_weather_name]] %||% "teal"
    box_icon <- WEATHER_ICONS[[current_weather_name]] %||% "cloud-sun-rain"
    
    valueBox(current_weather_name, "Current Weather", icon=icon(box_icon), color=box_color)
  })
  ...
  encyclopedia_data <- reactiveVal({
    message("--- Aggregating all encyclopedia data... ---")
    ...
    mutations_data <- all_mutation_data
  ...
  calculator_return <- calculator_server("calculator", 
                                         ...
                                         mutation_data = mutation_data_for_calculator)
